...
{ 
  ...
  uint16_t measuredLevel = ADC_ReadChannels(ADC_CHANNEL_0); 
	...
	rot = ((measuredLevel * Vdda) / 40950);	
	//Printf("Rotation = %0.2f, Vref=%d, measuredA0=%d\r\n", rot, Vdda, measuredLevel);
}

void HW_Get_Light( void ) 
{ 
  //uint32_t Vdda = 3000; //mV
	uint32_t Vdda = SYS_GetBatteryLevel();
  uint16_t measuredLevel = ADC_ReadChannels(ADC_CHANNEL_1); 
	...
	//degrees_c = ((measuredVal/4095)*(3.0*1000))/10;
	light = ((measuredLevel * Vdda) / 40950);	
	//Printf("Light = %0.2f, Vref=%d, measuredA0=%d\r\n", light, Vdda, measuredLevel);
}

void HW_Get_DHT11(void)
{
...
void dataSlice(volatile int data, volatile int i, volatile int limit) {
	if (i == limit)
		return ;
...
void combineData(void) {
	...
	for (volatile int i = 0; i < 5; i++) {	
		...
		dataSlice(data, 14 - (i * 3), 14 - (i * 3) - 3);
	}
}
